...
const trustLevelMap: Record<string, 'low' | 'medium' | 'high' | 'highest'> = {
  small_user: 'low',
 ...
 */
router.post('/register-with-email', async (req: Request, res: Response) => {
  try {
 ...
 * Update user profile
 */
router.put('/profile', verifyAuth, async (req: AuthRequest, res: Response) => {
  try {
    const user = users.find(u => u._id === req.user?.id);
    if (!user) {
      return res.status(404).json({ error: 'User not found' });

...
interface VerifiedItem extends IncomingItem {
  hubVerifiedAt: string;
}

export default function HubDashboard() {
  const { user, token, logout } = useAuth();
  const navigate = useNavigate();
  const [activeTab, setActiveTab] = useState<"incoming" | "verified" | "flagged">("incoming");
  ...
  const openVerifyDialog = (item: IncomingItem) => {
  ...
  const openFlagDialog = (item: IncomingItem) => {
    setSelectedItem(item);
    setFlagReason("");
    setFlagDialog(true);
  };
    ...
    <div className="min-h-screen bg-background">
      ...
      <main className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
        ...
        <div className="flex gap-2 mb-6 border-b border-border">
          {(["incoming", "verified"] as const).map((tab) => (
            <button
              key={tab}
                  ...
                  <tbody className="divide-y divide-border">
                    {verifiedItems.map((item) => (
                      <tr key={item._id} className="hover:bg-muted/20">
                        ...
                        <td className="px-4 py-3 text-sm text-muted-foreground">{(item as any).sourceUserName || "—"}</td>
                      </tr>
                    ))}
                  </tbody>
                </table>

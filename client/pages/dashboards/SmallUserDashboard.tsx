...
interface RewardData {
  ...
  milestones: Array<{ threshold: number; reached: boolean; rewardType: string }>;
}
...
export default function SmallUserDashboard() {
  ...
  const defaultFormItem = (): IntentItem => ({
  ...
  const [formItems, setFormItems] = useState<IntentItem[]>([defaultFormItem()]);
  ...
  const handleImageUpload = (itemIdx: number, e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    Array.from(files).forEach((file) => {
      ...
      reader.onloadend = () => {
        setFormItems((prev) => {
          ...
          updated[itemIdx] = {
            ...updated[itemIdx],
            photos: [...updated[itemIdx].photos, reader.result as string],
          };
          return updated;
        });
      };
